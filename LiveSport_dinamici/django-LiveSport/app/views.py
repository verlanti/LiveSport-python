...
class CalendarioView(generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        context = {'single_giornata_list': Calendario.objects.select_related().filter(campionato = kwargs['pk']).filter(giornata=re.split('(\d+|\D+)',request.POST['giornata'])[1],ar= re.split('(\d+|\D+)',request.POST['giornata'])[3]),
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {'single_giornata_list': Calendario.objects.select_related().filter(campionato = kwargs['pk']).filter(giornata=1,ar='A'),
                    'giornata_list': Calendario.objects.filter(campionato = kwargs['pk']).order_by('id')[::10],
...
def calc_classifica(campionato_id,squadre):
    ...
    for s in squadre:
        ...
        classifica.append( (s[1],vinte,38-vinte-perse,perse,golfatti,golsubiti,golfatti-golsubiti,vinte*3+38-vinte-perse) )
        vinte = 0
        perse = 0
        golfatti = 0
        golsubiti = 0
...
class SchedinaView(generic.ListView):
    ...
    def get(self, request, *args, **kwargs):
            ...
            """Argomenti passati al template: \
                    ...
                    giornata_list: ritorna le partite per giornata_list \
                    campionato_id: ritorna l'id del campionato"""
            context = {'giornata_list': Calendario.objects.select_related().filter(campionato = kwargs['pk']).order_by('id')[::10],
...
def match_team(campionato_id):
    ...
    for i in Calendario.objects.select_related().filter(campionato=campionato_id).order_by('id') :
            for p in i.risultati_set.all() :
                partite.append((p.partita.id,i.giornata,i.ar,i.data,i.locali.nome,i.ospiti.nome,p.retilocali,p.retiospiti))
